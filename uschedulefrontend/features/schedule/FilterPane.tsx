...
const FilterPane: React.FC<FilterPaneProps> = (props) => {
  // Detect external courses (owned by other programs)
  const externalCourses = useMemo(() => {
    if (!props.userProgramId) return [];
    return props.courseOfferings.filter(c => c.owningProgramId !== props.userProgramId);
  }, [props.courseOfferings, props.userProgramId]);

  const hasExternalCourses = externalCourses.length > 0;

  // Check if own courses are staffed
  const ownedCourses = useMemo(() => {
    if (!props.userProgramId) return props.courseOfferings;
    return props.courseOfferings.filter(c => c.owningProgramId === props.userProgramId);
  }, [props.courseOfferings, props.userProgramId]);

  const allOwnedCoursesStaffed = ownedCourses.length > 0 && ownedCourses.every(c => c.isAssigned);
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-amber-600 mt-1">
                    <strong>{externalCourses.length} course(s)</strong> are owned by other programs.
                    You can only assign instructors to courses your program owns.
                    After staffing your courses and scheduling, share a draft with the owning programs.
                  </p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {externalCourses.map(c => (
                      <span key={c.id} className="inline-flex items-center gap-1.5 px-2 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-lg">
                        <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                        {c.courseCode} ({c.owningProgramCode})
                      </span>
                    ))}
                  </div>
                </div>
            ...
            {props.courseOfferings.map(course => (
              <div key={course.id} className="relative group">
                <CourseCard
                  ...
                  isHead={true}
                  userProgramId={props.userProgramId}
                />
